...
func (q *queryStruct) OrWhere(column string, operator clauseoperators.Operators, args ...any) manipulateOrQuerying {
...
func (q *queryStruct) OrWhereBetween(column string, args ...any) manipulateOrQuerying {
	if len(q.whereClause) > 0 {
...
func (q *queryStruct) OrWhereIn(column string, args ...any) manipulateOrQuerying {
	if len(args) == 0 {
...
func (q *queryStruct) OrWhereExists(callback func() buildersub.SubQuery) manipulateOrQuerying {
	cb := callback()
	if cb == nil {
	...
	if len(q.whereClause) > 0 {
		q.whereClause = append(q.whereClause, fmt.Sprintf("OR EXISTS (%s)", subquery))
	} else {
...
func (q *queryStruct) OrWhereNull(column string) manipulateOrQuerying {
	if len(q.whereClause) > 0 {
...
func (q *queryStruct) OrWhereRaw(query string, args ...any) manipulateOrQuerying {
	if len(q.whereClause) > 0 {
...
func (q *queryStruct) OrWhereSub(column string, operator clauseoperators.Operators, callback func() buildersub.SubQuery) manipulateOrQuerying {
	...
	if len(q.whereClause) > 0 {
		q.whereClause = append(q.whereClause, fmt.Sprintf("OR %s %s (%s)", column, op, subquery))
	} else {
