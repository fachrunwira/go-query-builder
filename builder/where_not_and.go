...
func (q *queryStruct) WhereNot(column string, operator clauseoperators.Operators, args ...any) manipulateOrQuerying {
	...
	if placeholder != "" {
		if len(q.whereClause) > 0 {
			...
			q.whereClause = append(q.whereClause, fmt.Sprintf("NOT %s %s %s", column, op, placeholder))
		}
	} else {
		if len(q.whereClause) > 0 {
			q.whereClause = append(q.whereClause, fmt.Sprintf("AND NOT %s %s ?", column, op))
		} else {
			q.whereClause = append(q.whereClause, fmt.Sprintf("NOT %s %s ?", column, op))
		}
	}
...
func (q *queryStruct) WhereNotBetween(column string, args ...any) manipulateOrQuerying {
	if len(q.whereClause) > 0 {
...
func (q *queryStruct) WhereNotIn(column string, args ...any) manipulateOrQuerying {
	if len(args) == 0 {
...
func (q *queryStruct) WhereNotExists(callback func() buildersub.SubQuery) manipulateOrQuerying {
	cb := callback()
	if cb == nil {
	...
	if len(q.whereClause) > 0 {
		q.whereClause = append(q.whereClause, fmt.Sprintf("AND NOT EXISTS (%s)", subquery))
	} else {
...
func (q *queryStruct) WhereNotNull(column string) manipulateOrQuerying {
	if len(q.whereClause) > 0 {
...
func (q *queryStruct) WhereNotSub(column string, operator clauseoperators.Operators, callback func() buildersub.SubQuery) manipulateOrQuerying {
	...
	if len(q.whereClause) > 0 {
		q.whereClause = append(q.whereClause, fmt.Sprintf("AND NOT %s %s (%s)", column, op, subquery))
	} else {
